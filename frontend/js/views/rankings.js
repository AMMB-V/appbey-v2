// Rankings & Leaderboard View with Season Switching (Puntos vs Elo) & Live Search Filter
window.renderRankingsView = async (container) => {
  container.innerHTML = `<div class="text-center py-16 text-slate-500 flex flex-col items-center gap-3">
    <div class="w-8 h-8 border-2 border-cyan-400 border-t-transparent rounded-full animate-spin"></div>
    <span>Cargando Rankings Oficiales de la Asociación Panameña de Beyblade...</span>
  </div>`;
  ...
  try {
    ...
    const renderRankingContent = async (refreshFetch = true) => {
      ...
      const filteredData = rawRankingData.filter(item => {
      ...
      container.innerHTML = `
        <div class="space-y-6 max-w-6xl mx-auto pb-16">
          ...
          <!-- Podium of Top 3 (if in Season 1 and not filtered) -->
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
              ...
              <div class="glass-card rounded-2xl p-6 text-center border border-slate-600 md:order-1 flex flex-col justify-between space-y-3 bg-gradient-to-b from-slate-800/40 to-slate-950/80">
                ...
                <div class="relative inline-block mx-auto">
                  <img src="${top3[1].avatar_url || top3[1].user?.avatar_url || 'https://images.unsplash.com/photo-1570295999919-56ceb5ecca61?w=150'}" class="w-20 h-20 rounded-full border-4 border-slate-400 mx-auto object-cover shadow-lg"/>
                ...
                <div class="text-2xl font-black text-slate-200 font-mono">
                  ${isPoints ? `${top3[1].points} pts` : `${top3[1].elo || top3[1].elo_rating} elo`}
                </div>
              </div>
              ...
              <div class="glass-card rounded-2xl p-6 text-center border-2 border-amber-400 glow-gold md:order-2 flex flex-col justify-between space-y-3 bg-gradient-to-b from-amber-950/50 via-slate-900 to-slate-950">
                ...
                <div class="relative inline-block mx-auto">
                  <img src="${top3[0].avatar_url || top3[0].user?.avatar_url || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=150'}" class="w-24 h-24 rounded-full border-4 border-amber-400 mx-auto object-cover shadow-2xl"/>
                ...
                <div class="text-3xl font-black text-amber-400 font-mono">
                  ${isPoints ? `${top3[0].points} pts` : `${top3[0].elo || top3[0].elo_rating} elo`}
                </div>
              </div>
              ...
              <div class="glass-card rounded-2xl p-6 text-center border border-amber-700/60 md:order-3 flex flex-col justify-between space-y-3 bg-gradient-to-b from-amber-950/30 to-slate-950/80">
                ...
                <div class="relative inline-block mx-auto">
                  <img src="${top3[2].avatar_url || top3[2].user?.avatar_url || 'https://images.unsplash.com/photo-1527980965255-d3b416303d12?w=150'}" class="w-20 h-20 rounded-full border-4 border-amber-700 mx-auto object-cover shadow-lg"/>
                ...
                <div class="text-2xl font-black text-amber-500 font-mono">
                  ${isPoints ? `${top3[2].points} pts` : `${top3[2].elo || top3[2].elo_rating} elo`}
                </div>
              </div>
            </div>
          ` : ''}
          ...
          <div class="flex flex-col sm:flex-row items-center justify-between gap-3">
            <div class="relative w-full sm:w-80">
              ...
              <input
                ...
                class="w-full bg-slate-900/90 border border-slate-700 focus:border-cyan-400 text-white rounded-xl pl-9 pr-4 py-2.5 text-xs outline-none shadow"
              />
            </div>
            <div class="text-xs text-slate-400 font-mono">
              Mostrando <strong class="text-white">${filteredData.length}</strong> de <strong class="text-cyan-400">${rawRankingData.length}</strong> Bladers Oficiales
            </div>
          </div>
          ...
          <div class="glass-card rounded-2xl p-2 sm:p-4 overflow-x-auto border border-slate-800">
              ...
              <table class="w-full text-left text-xs sm:text-sm">
                ...
                <tbody class="divide-y divide-slate-800/60 font-mono text-xs">
                  ${filteredData.map((item, idx) => {
                    const bladerName = item.display_name || item.user?.display_name || 'Blader';
                    ...
                    return `
                      <tr onclick="showBladerModal(${JSON.stringify(item).replace(/"/g, '&quot;')})" class="hover:bg-slate-800/60 cursor-pointer transition">
                        <!-- Pos -->
                        ...
                        <td class="py-3 px-2 sm:px-3 font-sans">
                          <div class="flex items-center gap-2.5">
                            <img src="${avatar}" class="w-8 h-8 rounded-full object-cover border ${pos <= 3 ? 'border-amber-400' : 'border-slate-700'} shrink-0"/>
                            <div>
                        ...
                        <td class="py-3 px-2 text-center text-slate-300">
                          ...
                          <span class="text-[10px] text-slate-500 block">(${item.matches_played || (item.matches_won + item.matches_lost)} combates)</span>
                        </td>
                        ...
                        <td class="py-3 px-2 text-center">
                          <span class="px-2 py-0.5 rounded-full text-[11px] font-bold ${parseInt(item.win_rate) >= 60 ? 'bg-emerald-500/20 text-emerald-400' : parseInt(item.win_rate) >= 50 ? 'bg-cyan-500/20 text-cyan-400' : 'bg-slate-800 text-slate-400'}">
                            ${item.win_rate || '0%'}
                          </span>
                        </td>
                        ...
                        <td class="py-3 px-2 sm:px-3 text-right font-black ${isPoints ? 'text-amber-400 text-sm sm:text-base' : 'text-cyan-400 text-sm'}">
                          ${isPoints ? `${item.points} pts` : `${item.elo || item.elo_rating} elo`}
                        </td>
                      </tr>
                    `;
                  }).join("")}
            ...
            ` : `
              <div class="text-center py-12 space-y-3">
                ...
                <h3 class="text-base font-bold text-white">No se encontraron bladers con "${searchQuery}"</h3>
                <p class="text-xs text-slate-400 max-w-md mx-auto">
                  Prueba buscando por otro nombre o revisa la ortografía.
                </p>
                <button onclick="handleSearchInput('')" class="px-3 py-1.5 rounded-lg bg-slate-800 text-cyan-300 text-xs font-bold hover:bg-slate-700">
                  Limpiar Búsqueda
                </button>
              </div>
            `}
    ...
    window.showBladerModal = (b) => {
      ...
      const played = b.matches_played || (b.matches_won + b.matches_lost);
      ...
      modalEl.innerHTML = `
        <div class="glass-card w-full max-w-lg rounded-2xl border border-slate-700 p-6 space-y-5 bg-slate-900 shadow-2xl relative">
          <button onclick="document.getElementById('blader-detail-modal').remove()" class="absolute top-4 right-4 p-2 rounded-lg bg-slate-800 text-slate-400 hover:text-white">
            ✕
          </button>

          <div class="flex items-center gap-4">
            <div class="relative">
              <img src="${avatar}" class="w-16 h-16 rounded-full border-2 border-amber-400 object-cover"/>
              <span class="absolute -bottom-1 -right-1 px-2 py-0.5 rounded-full bg-amber-500 text-black text-[10px] font-black shadow">#${b.rank}</span>
            </div>
            <div>
          ...
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-2.5 text-center font-mono">
            <div class="bg-slate-950/80 p-3 rounded-xl border border-slate-800">
              ...
              <div class="text-lg font-black text-amber-400">${b.points}</div>
            </div>
            <div class="bg-slate-950/80 p-3 rounded-xl border border-slate-800">
              <div class="text-[10px] text-slate-400 uppercase font-sans">Win Rate</div>
              <div class="text-lg font-black text-emerald-400">${b.win_rate}</div>
            </div>
            <div class="bg-slate-950/80 p-3 rounded-xl border border-slate-800">
              <div class="text-[10px] text-slate-400 uppercase font-sans">Torneos</div>
              <div class="text-lg font-black text-white">${b.tournaments_played}</div>
            </div>
            <div class="bg-slate-950/80 p-3 rounded-xl border border-slate-800">
          ...
          <div class="space-y-2 text-xs text-slate-300 bg-slate-950/40 p-4 rounded-xl border border-slate-800">
            <div class="flex justify-between py-1 border-b border-slate-800">
              ...
              <span class="font-mono font-bold text-emerald-400">${b.matches_won} Ganadas <span class="text-slate-500">/</span> <span class="text-rose-400">${b.matches_lost} Perdidas</span></span>
            </div>
            <div class="flex justify-between py-1 border-b border-slate-800">
              <span class="text-slate-400">Puntos a Favor / En Contra:</span>
              <span class="font-mono font-bold">${b.points_for} Favor / ${b.points_against} Contra (${diff >= 0 ? '+' : ''}${diff})</span>
            </div>
            <div class="flex justify-between py-1 border-b border-slate-800">
              <span class="text-slate-400">Puntos Bonus Acumulados:</span>
              <span class="font-mono font-bold text-amber-300">+${b.bonus_points} pts</span>
            </div>
            <div class="flex justify-between py-1">
              <span class="text-slate-400">Advertencias (Warnings):</span>
              <span class="font-mono font-bold text-slate-400">${b.warnings}</span>
            </div>
          </div>
